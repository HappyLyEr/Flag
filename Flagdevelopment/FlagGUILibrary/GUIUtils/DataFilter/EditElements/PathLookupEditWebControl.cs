...
namespace GASystem.GUIUtils.DataFilter.EditElements
{
	...
	/// </summary>
	public class PathLookupEditWebControl :IEditWebControl, System.Web.UI.INamingContainer
	{
		...
		public PathLookupEditWebControl(FieldDescription fd)
		{
			//
        ...
        private ArrayList getPathData()
        {
            ArrayList list = new ArrayList();

            try
            {
                GADataClass lookupTable = GADataRecord.ParseGADataClass(_fd.LookupTable);
                ClassDescription cd = ClassDefinition.GetClassDescriptionByGADataClass(lookupTable);


                BusinessLayer.BusinessClass bc = RecordsetFactory.Make(lookupTable);
                DataSet ds = bc.GetAllRecordsWithinOwnerAndLinkedRecords(SessionManagement.GetCurrentDataContext().InitialContextRecord);

                if (ds.Tables.Count > 0) 
                    foreach (DataRow row in ds.Tables[0].Rows) 
                    {
                        string liText = row[cd.ObjectName].ToString();
                        string liValue = "/" + lookupTable.ToString() + "-" + row[lookupTable.ToString().Substring(2) + "rowid"].ToString() + "/";
