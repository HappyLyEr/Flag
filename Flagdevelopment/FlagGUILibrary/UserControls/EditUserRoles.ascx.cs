...
namespace GASystem.UserControls
{
	...
	/// </summary>
	public class EditUserRoles : System.Web.UI.UserControl
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			int rowId = 0; GADataClass dataClass = GADataClass.NullClass;

			if (null!=Request["RowId"] && GAUtils.IsNumeric(Request["RowId"]))
				rowId = int.Parse(Request["RowId"].ToString());
			
			if (null!=Request["DataClass"] && GAUtils.IsGADataClass(Request["DataClass"]) )
				dataClass = GADataRecord.ParseGADataClass(Request["DataClass"].ToString());
			
			if (rowId!=0 && dataClass!=GADataClass.NullClass)
				_dataRecord = new GADataRecord(rowId, dataClass);
			else
				_dataRecord = null;

			if (!Page.IsPostBack && _dataRecord!=null)
				SetupGrid();

			//bool hasUpdate = Security.HasUpdatePermission(_dataRecord);
			//bool hasCreate = Security.HasUpdatePermission(_dataRecord);
			//testSqlGenerate();
			DataAccess.Security.GASecurityDb.GetUserEngagementsAsArray();
		}
		...
		private void SetupGrid()
		{
			try
			{
				...
				DataRecordRolePermissionsDS rolePermissionsSet = Security_old.GetSecurityRolesAccessForDataRecord(_dataRecord);
			...
			if (e.CommandName == "Insert")
			{
				try 
				{
					...
					if (rolesDropDown!=null && null!=rolesDropDown.SelectedItem)
					{
						...
						Security_old.AddDataRecordRolePermissions(_dataRecord, ddlItem.Value, hasRead, hasUpdate, hasCreate, hasDelete);
		...
		private void grdDataRecordRolePermissions_DeleteCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			try
			{
				DataRecordRolePermissionsDS rolePermissionsData = Security_old.GetSecurityRolesAccessForDataRecord(_dataRecord);
				...
				Security_old.UpdateDataRecordRolePermissions(rolePermissionsData, _dataRecord);
				SetupGrid();
			}
			catch (GAException GAex) 
			{
		...
		private void grdDataRecordRolePermissions_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			try
			{
				...
				Security_old.UpdateDataRecordRolePermissions(_dataRecord, key, hasRead, hasUpdate, hasCreate, hasDelete);
