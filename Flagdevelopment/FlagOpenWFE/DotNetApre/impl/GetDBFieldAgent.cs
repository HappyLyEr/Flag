...
//			System.Console.WriteLine("Starting Get Database Field Agent");
            ...
            System.Console.WriteLine("Get Database Field Agent Class: "+DataClass+" Field: "+fieldName);
			...
			if (DataClass != string.Empty)  // && !DataClass.Equals(owner.DataClass.ToString()))
			{
				...
				while (!DataClass.ToUpper().Equals(owner.DataClass.ToString().ToUpper()))
				{
					...
					if (curretLevel >= MAXNUMBEROFLEVELS)
                        throw new GASystem.GAExceptions.GAException("Maximun number of recursive searches for ownerclass " + DataClass.ToString() + "exceeded");
				}
			}
			...
			if (fd == null) 
			{
				...
				//TODO throw error ??
                throw new GASystem.GAExceptions.GAException("Owner dataclass " + DataClass.ToString() + " fieldname " + fieldName + "is not defined in fielddescription");
			...
			if (ds.Tables[0].Rows.Count == 0)
                throw new GASystem.GAExceptions.GAException("Owner record " + DataClass.ToString() + "not found above "+owner.DataClass.ToString() +"with rowId: "+owner.RowId.ToString());
            ...
            else if (fd.ControlType.ToUpper() == "LOOKUPFIELDGROUPS" && fd.LookupTable.ToUpper() == "GALISTS")
            {
                // 多选框返回多个分号分隔的值
                BusinessClass bc = GASystem.BusinessLayer.Utils.RecordsetFactory.Make(owner.DataClass);
                System.Data.DataSet dsraw = bc.GetByRowId(owner.RowId);
                if (dsraw.Tables[0].Rows.Count > 0 && dsraw.Tables[0].Columns.Contains(fieldName))
                {
                    if (!dsraw.Tables[0].Rows[0].IsNull(fieldName))
                    {
                        if (dsraw.Tables[0].Rows[0][fieldName] != null)
                        {
                            string[] ids = dsraw.Tables[0].Rows[0][fieldName].ToString().Split(';');
                            foreach (string id in ids)
                            {
                                codeValue += Lists.GetListValueByRowId(int.Parse(id), null) + ";";
                            }
                            codeValue.TrimEnd(';');
                        }
                    }
