...
namespace GASystem.DataAccess
{
	...
	/// </summary>
    public class DataAccessReportViewForViews : IDataAccessReportView
	{
		private  string _selectSqlTemplate = @"SELECT * FROM {0} ";
        private  string _selectSqlTemplateRowId = @"SELECT * FROM {0} where {1} = {2} ";
        private  string _selectSqlTemplateRowIds = @"SELECT * FROM {0} where {1} in ({2}) ";
        ...
        public DataAccessReportViewForViews(GADataClass DataClass, GADataTransaction Transaction)
		{
        ...
        DataSet IDataAccessReportView.GetByRowId(int rowId)
        {
            ...
            //TODO get top owner from configuration file
            _selectSql = string.Format(_selectSqlTemplateRowId, _dataClass.ToString(), _dataClass.ToString().Substring(2) + "rowId", rowId.ToString());
            SqlDataAdapter da = new SqlDataAdapter(_selectSql, myConnection);
            da.Fill(GAData, _dataClass.ToString());
            return GAData;
        }
        ...
        DataSet IDataAccessReportView.GetByRowIds(ArrayList rowIds)
        {
            ...
            //TODO get top owner from configuration file
            _selectSql = string.Format(_selectSqlTemplateRowIds, _dataClass.ToString(), _dataClass.ToString().Substring(2) + "rowId", GASystem.AppUtils.GAUtils.ConvertArrayToString(rowIds.ToArray(), ","));
            SqlDataAdapter da = new SqlDataAdapter(_selectSql, myConnection);
            da.Fill(GAData, _dataClass.ToString());
            return GAData;
