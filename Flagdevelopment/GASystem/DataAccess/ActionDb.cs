...
namespace GASystem.DataAccess
{
	...
	public class ActionDb
	{
		...
		public static ActionDS GetActionForWorkFlowStartPending()
		{
            ...
            // Tor 20190326 : do not return actions owned (serch GASuperClass.Path) by GALocation where SwitchFree3=1
                    // Tor 20120507 : create select for ensuring that StartPending Actions have an owner
            string _filterForSelectSqlStartPending = @"select ' Select s.SuperClassRowId '
...
";
            ...
            // Tor 20190326 added filter to exclude actions owned (search GASuperClass.Path) by GALocation where SwitchFree3=1
            string _filterForSelectSqlStartPendingExcludingLocations = "select 's.Path not like ''%/GALocation-'+CAST(l.LocationRowId as nvarchar(100))+'/%'' and ' from GALocation l where l.SwitchFree3=1 ";
            System.Data.DataSet dsl = new System.Data.DataSet();
            SqlDataAdapter dafl = new SqlDataAdapter(_filterForSelectSqlStartPendingExcludingLocations, myConnection);
            dafl.Fill(dsl);
            string locationSqlFilter = "";
            foreach (System.Data.DataRow row in dsl.Tables[0].Rows)
                locationSqlFilter+= row[0];
            if (locationSqlFilter != "") locationSqlFilter=" where ( "+locationSqlFilter.Substring(0,locationSqlFilter.Length-4)+" )";
            // added locationSqlFilter
