...
namespace GASystem.AppUtils
{
	...
	public class FieldDefintion
	{
        ...
        // Tor 20150529 : added field GAFieldDefinitions.nTextFree1 for CopyFromFieldId attributes that are to long to be stored in columne CopyFromFieldId (nvachar(200)) 

        private static string _sql = @"SELECT  col.character_maximum_length AS Length, col.DATA_TYPE as ColumnType,  GAFieldDefinitions.TableId, GAFieldDefinitions.FieldId, GAFieldDatatype.DataType, GAFieldDatatype.InheritFrom, GAFieldDatatype.CaptionWidth, 
...
,GAFieldDefinitions.nTextFree1,GAFieldDefinitions.IntFree2
FROM GAFieldDatatype INNER JOIN GAFieldDefinitions ON GAFieldDatatype.DataType = GAFieldDefinitions.Datatype
left join INFORMATION_SCHEMA.COLUMNS col on col.TABLE_NAME = GAFieldDefinitions.TableId and col.COLUMN_NAME = GAFieldDefinitions.FieldId";
        ...
        // Tor 20140630 added method to check if current logged on user has role in list (list format ;role;role;...)
        public static bool IfFieldExists(string DataClass, string FieldName)
        {
            FieldDescription fd = GetFieldDescription(FieldName, DataClass);
		...
		public static FieldDescription GetFieldDescription(DataRow FieldDefinitionRow)
        {
            ...
            if (FieldDefinitionRow["IntFree2"] != DBNull.Value)
                fd.IsLookupForTranslation = FieldDefinitionRow["IntFree2"].Equals(0);

            // Tor 20150529 get CopyFromFieldId string from other GAFieldDefinitions column. Example: CopyFromFieldId=<%ref nTextFree1%>
            fd.CopyFromFieldId = FieldDefinitionRow["CopyFromFieldId"].ToString();
            if (FieldDefinitionRow["CopyFromFieldId"] != DBNull.Value)
            {
                if (fd.CopyFromFieldId.Length > 7 && fd.CopyFromFieldId.IndexOf("<%ref:") == 0)
                {
                    string a = fd.CopyFromFieldId.Replace("<%ref: ", "").Replace("%>", "").Replace(" ", "");
                    fd.CopyFromFieldId = FieldDefinitionRow[a].ToString();
                }
            }
            ...
            if (FieldDefinitionRow["SwitchFree3"] != DBNull.Value) fd.ReportOnFieldUpdate = (bool)FieldDefinitionRow["SwitchFree3"];
            if (FieldDefinitionRow["IntFree1"] != DBNull.Value && (fd.LookupTable == "GAPersonnel" ||
                // Tor 20170313 Responsible changed from Role ER to Title TITL fd.ListCategory = "ER";
                (fd.ListCategory == "TITL" || 
                // Tor 20170313 lookup win GALists for assigned personnel 
                (fd.LookupTable == "GALists" && fd.LookupFilter.ToUpper().IndexOf("GAEMPLOYMENT-JOBDESCRIPTION")>0))
                )) fd.ReportUpdateTo = true;
            // Tor 20150626 added field ColumnType from information schema (values int, nvarchar etc)
            fd.ColumnType = FieldDefinitionRow["ColumnType"].ToString();
	...
	public class FieldDescription : object, IComparable
	{
        ...
        public string ColumnType=string.Empty; //COL_LENGTH() Type of database column (int, ntext etc)
        ...
        public bool IsLookupForTranslation = false;
