...
namespace GASystem.AppUtils.LookupFilterGenerator
{
	...
	public class SiblingLookupFilter : GeneralLookupFilter
	{
        ...
        const string _COMMONLOCATION_WHERE_STATEMENTBASE = @"{0} in (select distinct {1} from {2} e inner join GASuperClass gs on e.{3} = gs.MemberClassRowId and gs.MemberClass='{2}'
                                                            inner join GASuperClass cs on cs.MemberClass='{4}'and cs.OwnerClassRowId = gs.OwnerClassRowId
                                                            where cs.MemberClassRowId = {5} and {6} )";
                        //{0} = <lookupclassrowidfield>
                        //{1} = <siblingfield>
                        //{2} = <siblingclass>
                        //{3} = <siblingclassrowidfield> 
		...
		public SiblingLookupFilter(GADataClass LookupDataClass, GADataRecord LookupOwner, string LookupFilter, string LookupField, bool isAllowed)
		{
			Owner = LookupOwner;
            try 
			{
                ...
                if (isAllowed)
                {
                    ...
                    filter = string.Format(_WHERE_STATEMENTBASE, LookupField, _siblingField, _siblingClass.ToString(), siblingClassRowIdField, _owner.DataClass.ToString(), _owner.RowId.ToString(), getDateFilter());
                }
                else
                {
                    filter = string.Format(_COMMONLOCATION_WHERE_STATEMENTBASE, LookupField, _siblingField, _siblingClass.ToString(), siblingClassRowIdField, _owner.DataClass.ToString(), _owner.RowId.ToString(), getDateFilter());
                }
                Filter = filter;
			} 
			catch (Exception ex) 
